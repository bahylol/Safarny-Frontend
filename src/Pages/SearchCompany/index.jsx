...
const SearchCompanies = () => {
    ...
    const searchUsers = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/search/company/${selectedCategory}/${searchQuery}`, {
                method: 'GET',
                headers: {
                    ...
                    {users.map((user, index) => (
                        <div key={index} className="p-6 sm:p-12 dark:bg-gray-900 dark:text-gray-100 shadow-md rounded-md mb-4">
                            <div className="flex flex-col md:flex-row">
                                <img src={require(`../../assets/Images/company${user.avatar}.jpg`)}
                                    alt={user.fname} className="self-center flex-shrink-0 w-24 h-24 border rounded-full md:mr-6 md:dark:bg-gray-500 md:dark:border-gray-700" />
                                <div className="flex-grow">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <button
                                            ...
                                            onClick={() => window.location.href = `/profile/company?userId=${user.id}`}
                                        >
                                            <PersonIcon />
                                        </button>
                                    </div>
                                    <p className="mt-2 dark:text-gray-400">{user.about}</p>
                                </div>
                            </div>
                        </div>
                    ))}
