...
function MyQuotes() {
    ...
    for (let i = 0; i < rowsData.length; i++) {
        const { fname, destination, price, start_date, duration, type, status } = rowsData[i];
        const dataObject = createData(fname, destination, price, formatDate(start_date).split("at")[0], duration + " days", type, status);
        rows.push(dataObject);
    }

    function createData(fname, destination, price, start_date, duration, type, status) {
        return { fname, destination, price, start_date, duration, type, status };
    }
