...
const SearchUsers = () => {
    ...
    const searchUsers = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/search/traveler/${selectedCategory}/${searchQuery}`, {
                method: 'GET',
                headers: {
                    ...
                    {users.map((user, index) => (
                        <div key={index} className="p-6 sm:p-12 dark:bg-gray-900 dark:text-gray-100 shadow-md rounded-md mb-4">
                            <div className="flex flex-col md:flex-row">
                                <img src={require(`../../assets/Images/traveler${user.avatar}.jpg`)}
                                ...
                                <div className="flex-grow">
                                    <div className="flex justify-between items-center">
                                        <h4 className="text-lg font-semibold md:text-left">{user.fname + " " + user.lname}</h4>
                                        <button
                                            ...
                                            onClick={() => window.location.href = `/profile/traveler?userId=${user.id}`}
                                        >
                                            <PersonIcon />
                                        </button>
                                    </div>
                                    <p className="mt-2 dark:text-gray-400">{user.aboutme}</p>
                                </div>
                            </div>
                        </div>
                    ))}
